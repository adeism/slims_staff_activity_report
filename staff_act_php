...
// system log for update bibliographic, item, mmember as part of the report &  added bound parameter for prevent from sql injection   | github.com/adeism
function showBiblioEntries($obj_db, $array_data)
{
    global $start_date, $until_date;
    $sql = 'SELECT COUNT(log_id) FROM system_log WHERE log_location=\'bibliography\' AND log_type=\'staff\' AND (log_msg LIKE \'%insert bibliographic data%\' OR log_msg LIKE \'%update bibliographic data%\') AND id=? AND TO_DAYS(log_date) BETWEEN TO_DAYS(?) AND TO_DAYS(?)';
    $stmt = $obj_db->prepare($sql);
    $stmt->bind_param('sss', $array_data['2'], $start_date, $until_date);
    $stmt->execute();
    $_count_d = $stmt->get_result()->fetch_row();
    return $_count_d[0];
}
...
{
    global $start_date, $until_date;
    $sql = 'SELECT COUNT(log_id) FROM system_log WHERE log_location=\'bibliography\' AND log_type=\'staff\' AND (log_msg LIKE \'%insert item data%\' OR log_msg LIKE \'%update item data%\') AND id=? AND TO_DAYS(log_date) BETWEEN TO_DAYS(?) AND TO_DAYS(?)';
    $stmt = $obj_db->prepare($sql);
    $stmt->bind_param('sss', $array_data['3'], $start_date, $until_date);
    $stmt->execute();
    $_count_d = $stmt->get_result()->fetch_row();
    return $_count_d[0];
}
...
{
    global $start_date, $until_date;
    $sql = 'SELECT COUNT(log_id) FROM system_log WHERE log_location=\'membership\' AND log_type=\'staff\' AND (log_msg LIKE \'%add new member%\' OR log_msg LIKE \'%update member data%\') AND id=? AND TO_DAYS(log_date) BETWEEN TO_DAYS(?) AND TO_DAYS(?)';
    $stmt = $obj_db->prepare($sql);
    $stmt->bind_param('sss', $array_data['4'], $start_date, $until_date);
    $stmt->execute();
    $_count_d = $stmt->get_result()->fetch_row();
    return $_count_d[0];
}
...
function showCirculation_Loan($obj_db, $array_data)
{
    ...
    $sql = 'SELECT COUNT(log_id) FROM system_log WHERE log_location=\'circulation\' AND log_type=\'member\' AND (log_msg LIKE CONCAT(?, \'%new loan%\')) AND TO_DAYS(log_date) BETWEEN TO_DAYS(?) AND TO_DAYS(?)';
    $stmt = $obj_db->prepare($sql);
    $stmt->bind_param('sss', $_user, $start_date, $until_date);
    $stmt->execute();
    $_count_d = $stmt->get_result()->fetch_row();
    return $_count_d[0];
}
...
function showCirculation_Return($obj_db, $array_data)
{
    global $start_date, $until_date;
    $_user = $obj_db->escape_string($array_data[0]);
    $sql = 'SELECT COUNT(log_id) FROM system_log WHERE log_location=\'circulation\' AND log_type=\'member\' AND (log_msg LIKE CONCAT(?, \'%return item%\') OR log_msg LIKE CONCAT(?, \'%Quick Return%\')) AND TO_DAYS(log_date) BETWEEN TO_DAYS(?) AND TO_DAYS(?)';
    $stmt = $obj_db->prepare($sql);
    $stmt->bind_param('ssss', $_user, $_user, $start_date, $until_date);
    $stmt->execute();
    $_count_d = $stmt->get_result()->fetch_row();
    return $_count_d[0];
}
...
function showCirculation_Extends($obj_db, $array_data)
{
    global $start_date, $until_date;
    $_user = $obj_db->escape_string($array_data[0]);
    $sql = 'SELECT COUNT(log_id) FROM system_log WHERE log_location=\'circulation\' AND log_type=\'member\' AND (log_msg LIKE CONCAT(?, \'%extend loan%\')) AND TO_DAYS(log_date) BETWEEN TO_DAYS(?) AND TO_DAYS(?)';
    $stmt = $obj_db->prepare($sql);
    $stmt->bind_param('sss', $_user, $start_date, $until_date);
    $stmt->execute();
    $_count_d = $stmt->get_result()->fetch_row();
